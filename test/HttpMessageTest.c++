...
{
    void PrintTo(const HttpMessage& message, ostream* stream)
    {
...
int getDataChunk(const std::string& wholeData, char* chunkBuffer, int chunkSize, int currentChunk)
{
    int copySize = 0;
    int currentPos = currentChunk * chunkSize;
    if (currentPos < wholeData.length())
    {
        copySize = currentPos + chunkSize > wholeData.length() ? (int)wholeData.length() - currentPos : chunkSize;
...
{
    //given we have a http request with just the metadata
    std::string dataToStream = "POST /an_endpoint HTTP/1.1";
    int currentChunk = 0;
...
{
    //given we have a http request without an endpoint
