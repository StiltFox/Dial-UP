...
namespace StiltFox::DialUp::Tests::PortAuthorityTests::StartApplication
{
    ...
    void setLoggerOnPortAuthority(PortAuthority& authority, LogMap& logMap)
    {
        authority.logger = [&logMap](PortAuthority::LogSevarity logLevel, string message)
        {
            logMap[logLevel].push_back(message);
        };
    }

    bool checkLogMapForValue(const PortAuthority::LogSevarity& logLevel, const LogMap& logMap, const string& value)
    {
    ...
    TEST(StartApplication, will_print_a_custom_banner_to_standard_console_output_when_callled_with_a_banner)
    {
        ...
        //when we start the application without a custom banner
        authority.startApplication("this is a test banner");
    ...
    TEST(StartApplication, will_print_an_info_level_log_stating_what_port_the_application_is_running_on)
    {
        ...
        PortAuthority authority(110,100,10);
        LogMap logMap;
        setLoggerOnPortAuthority(authority, logMap);
    ...
    TEST(StartApplicaiton, will_print_an_error_level_log_stating_that_the_kill_port_cannot_be_opened)
    {}
}